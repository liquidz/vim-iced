...
function! s:parse_aliases_value(v) abort
  ...
  let v = iced#compat#trim(a:v)
  for pair in split(iced#compat#trim(a:v), ',')
    let [alias, name] = split(pair, '(')
    let result[iced#compat#trim(alias)] = split(substitute(name, '[()]', '', 'g'), ' \+')
  endfor
  return result
endfunction
...
function! s:namespace_aliases(resp) abort
  ...
  for grp in split(aliases, '}')
    ...
    let k = strpart(iced#compat#trim(k), 1)
